...
Im zweiten Ansatz widmen wir uns dem Modell aus \cite{algo2}. Dieser Ansatz arbeitet mit abstrakten Sicherheitslevels. Im Folgenden werden zwar Skalare hierfür verwendet, was allerdings nicht zwingend so vorgesehen ist. Man könnte stattdessen auch komplexere Sicherheitsvektoren verwenden, welche bei Weitem detailliertere Sicherheitsmerkmale beschreiben. Des weiteren verfügt jeder physische wie auch virtuelle Knoten über zwei Sicherheitswerte: Anforderungslevel und (eigenes) Sicherheitslevel. Der Anforderungslevel definiert das Minimum-Level des Gegenübers, der Sicherheitslevel definiert die eigenen gewünschten Sicherheitsmerkmale. Virtuelle Links verfügen über Anforderungslevels, physische Links nur über Sicherheitslevels. Die vorausgesetzten Basisanforderungen, welchen alle VNRs unterliegen, beschränken sich auf 4 Regeln, welche mittels der zuvor genannten Merkmale umgesetzt werden:

\begin{lstlisting}
1) Ein physischer Knoten sollte einen Sicherheitslevel garantieren, 
   der höher ist als die Anforderungen der darauf abzubildenden
   virtuellen Knoten.

2) Der Sicherheitslevel des virtuellen Knotens sollte höher sein, 
   als das Anforderungslevel des physichen Knotens.

3) Alle virtuellen Knoten, welche auf den selben physischen Knoten
   abgebildet werden, sollten über einen ausreichenden Sicherheitslevel
   verfügen.

4) Der Anforderungslevel des virtuellen Links sollte stets niedriger
...
Der zugehörige Abbildungsalgorithmus bietet nun zwei Optionen: uSAV und cSAV. Im Gegensatz zu uSAV werden bei cSAV Knoten und Links separat voneinander behandelt. !!!Weitere Unterschiede!!! 5 Zeilen ca..
Beide Algorithmen arbeiten allerdings mit den selben Sub-Algorithmen: Node Mapping und Link Mapping
...
FEHLT [GG]: Optimierung der Grafiken, Erklärung der Ansatz2-Algorithmen, Ansatz2 Komplexitung und Performanz, 
